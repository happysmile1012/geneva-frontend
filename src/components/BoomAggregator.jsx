...
export default function BoomAggregator() {
  ...
  const loadProductContent = async (id) => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/product-history/product-content`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    <div className="flex h-screen font-goudy">
      ...
      >
        <div className="flex flex-col items-center gap-3 overflow-y-auto">
          <div
            className="absolute top-0 right-0 px-2 pt-3 cursor-pointer"
            onClick={() => setHideSideBar(true)}
          >
            <svg
              viewBox="0 0 24 24"
          ...
          />
          <img
            src="/image/llm-icons.png"
            alt="x"
            className="w-[80%] cursor-pointer"
          />
          <div align="center">
            <img
              src="/image/bottom-button.png"
        ...
        {loadingHistory ? (
          ...
          <div className="w-full px-4 gap-2 flex flex-col mt-5">
            {Array.isArray(productHistory) &&
              productHistory
                .filter((item) => item.query.indexOf(search) > -1)
                .map((item, index) => {
                  return (
                    <div key={index} className="relative group w-full">
                      <Button
                        ...
                        style={{
                          background:
                            index == selectedHistoryId ? "#98c0e5" : "#DCEAF7",
                        }}
                          ...
                          <DialogFooter className="gap-20 flex !justify-center">
                            ...
                            <DialogClose>
                              <div
                                ...
                                onClick={() =>
                                  deleteHistory(item.id, item.user_id)
                                }
                              >
                                yes
                              </div>
            ...
            {Array.isArray(productHistory) &&
              productHistory.filter((item) => item.query.indexOf(search) > -1)
                .length == 0 && (
      ...
      >
        <header>
          <div className="flex justify-between items-center p-2">
            <div className="flex gap-3">
              ...
              <div
                className="w-[60px] bg-black text-white text-center rounded-sm py-1 shadow-[2px_2px_2px_black] cursor-pointer"
                onClick={gotoBack}
              >
                back
              </div>
            ...
            {!isMobile && (
              <span className="text-3xl underline decoration-[#189453] decoration-2 underline-offset-[10px] self-end text-center">
                Discover & Shop anything consumer. Powered by Boom.
              </span>
            )}
            <img
              src="/image/boom.png"
              alt="x"
              className="w-[60] self-end cursor-pointer"
            />
          </div>
          {isMobile && (
            <div
              className=" mt-[20px] text-3xl underline decoration-[#189453] decoration-2 underline-offset-[10px] self-end text-center"
        ...
        />
        <div
          className=" w-[90%] m-auto flex  justify-center md:justify-around lg:justify-around items-center text-2xl my-3"
          style={{ flexWrap: "wrap", gap: "25px" }}
        >
          <div
            className={`${
              category == "footwear"
                ? "underline decoration-[#189453] decoration-2 underline-offset-[10px]"
                : ""
            } cursor-pointer`}
        ...
        {loadingContent ? (
          ...
          <div className="w-full  p-8">
            {productType === "specific" &&
              ...
              products.length > 0 && (
                <>
                  <div className="text-2xl">Best match for you:</div>
            ...
            <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 justify-items-center">
              {Array.isArray(products) &&
                (productType === "specific" ? products.slice(1) : products).map(
                  (item, index) => (
                    <ProductSlider
                      key={index}
                      linkToProduct={linkToProduct}
...
function ProductSlider({ image, thumbnails, url, linkToProduct, price, id }) {
  ...
  const { theme, setTheme } = useTheme();
  return (
    <div className="flex items-center gap-[15] align-center" id={id}>
      {thumbnails && thumbnails.length > 1 && (
        <div>
          <img
            src={`/image/${theme == "light" ? "dark" : "white"}-left.png`}
            width={20}
            height={20}
            onClick={() => {
      ...
      {thumbnails && thumbnails.length > 1 && (
        <div>
          <img
            src={`/image/${theme == "light" ? "dark" : "white"}-right.png`}
            width={20}
            height={20}
            onClick={() => {
              setIndex((prevIndex) => (prevIndex + 1) % thumbnails.length);
